...
class Dealership: DealershipProtocol {
    ...
    func addToShowroom(car: CarProtocol) {
        ...
        guard let index = presaleService(car: car) else {
            print("Машина не найдена при выполнении препродажной подготовки: \(car)")
            return
        }
...
extension Dealership: SpecialOffer {
   ...
   private func accEmergencyAccessory(to car: inout CarProtocol) {
    ...
    private func makeSpecialOfferToCar(to car: inout CarProtocol) -> Bool {
